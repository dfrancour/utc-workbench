...
import type { Event, ParsedTimestamp, SessionState } from '../types';
...
describe('renameSession', () => {
  ...
  it('preserves createdAt and events on rename', () => {
    const state = createSession(EMPTY_SESSION_STATE, 'initial');
    const id = state.activeSessionId!;
    const withEvent = updateActiveSessionEvents(state, (events) =>
      addEvent(events, parsed(1_000), 'tag')
    );
...
describe('updateActiveSessionEvents', () => {
  it('applies the updater to the active session events only', () => {
    ...
    const next = updateActiveSessionEvents(state, (events) =>
      addEvent(events, parsed(1_000), 'tag')
    );
  ...
  it('can remove events', () => {
    ...
    const trimmed = updateActiveSessionEvents(state, (events) =>
      events.filter((e: Event) => e.label === 'a')
    );
  ...
  it('promotes a draft session on first non-empty mutation', () => {
    ...
    const promoted = updateActiveSessionEvents(draftState, (events) =>
      addEvent(events, parsed(1_000), 'first')
    );
